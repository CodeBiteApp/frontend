import { DobiAnimatedProps } from "@/types/dobi";
import { useEffect, useRef } from "react";
import { Animated, Easing } from "react-native";
import { G, Path, Rect, Svg } from "react-native-svg";

const AnimatedG = Animated.createAnimatedComponent(G);

export default function DobiCodingAnimated({ size = 300 }: DobiAnimatedProps) {
  const leftArmY = useRef(new Animated.Value(0)).current;
  const rightArmY = useRef(new Animated.Value(0)).current;
  const tailAngle = useRef(new Animated.Value(0)).current;
  const eyeAnim = useRef(new Animated.Value(0)).current;

  // 0 -> 1 로 갈 때 x:+4, y:+5 이동
  const eyeX = eyeAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [0, 4],
  });

  const eyeY = eyeAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [0, 5],
  });

  useEffect(() => {
    const leftTyping = Animated.loop(
      Animated.sequence([
        Animated.timing(leftArmY, {
          toValue: -15,
          duration: 150,
          easing: Easing.out(Easing.quad),
          useNativeDriver: false,
        }),
        Animated.timing(leftArmY, {
          toValue: 0,
          duration: 150,
          easing: Easing.in(Easing.quad),
          useNativeDriver: false,
        }),
      ]),
    );

    const rightTyping = Animated.loop(
      Animated.sequence([
        Animated.timing(rightArmY, {
          toValue: -15,
          duration: 150,
          easing: Easing.out(Easing.quad),
          useNativeDriver: false,
        }),
        Animated.timing(rightArmY, {
          toValue: 0,
          duration: 150,
          easing: Easing.in(Easing.quad),
          useNativeDriver: false,
        }),
      ]),
    );

    const tailSway = Animated.loop(
      Animated.sequence([
        Animated.timing(tailAngle, {
          toValue: 2,
          duration: 600,
          easing: Easing.inOut(Easing.sin),
          useNativeDriver: false,
        }),
        Animated.timing(tailAngle, {
          toValue: -2,
          duration: 600,
          easing: Easing.inOut(Easing.sin),
          useNativeDriver: false,
        }),
      ]),
    );

    const eyeGlance = Animated.loop(
      Animated.sequence([
        Animated.delay(2500),
        Animated.timing(eyeAnim, {
          toValue: 1,
          duration: 180,
          easing: Easing.out(Easing.quad),
          useNativeDriver: false,
        }),
        Animated.delay(300),
        Animated.timing(eyeAnim, {
          toValue: 0,
          duration: 180,
          easing: Easing.in(Easing.quad),
          useNativeDriver: false,
        }),
      ]),
    );

    leftTyping.start();
    // 오른팔을 150ms 늦게 시작해서 양팔이 교대로 타자치는 효과
    const offset = setTimeout(() => rightTyping.start(), 120);
    tailSway.start();
    eyeGlance.start();

    return () => {
      clearTimeout(offset);
      leftTyping.stop();
      rightTyping.stop();
      tailSway.stop();
      eyeGlance.stop();
    };
  }, []);

  return (
    <Svg width={size} height={size} viewBox="0 0 500 484" fill="none">
      <Path
        d="M348 138C337.728 135.867 301.136 128.4 296 122"
        stroke="#1A1A1A"
        strokeWidth={5}
        strokeLinecap="round"
      />
      <Path
        d="M205.995 159.754C213.528 155.254 213.062 139.83 204.956 125.303C196.849 110.776 184.171 102.648 176.639 107.147C169.106 111.646 169.572 127.07 177.678 141.597C185.785 156.124 198.463 164.253 205.995 159.754Z"
        fill="#8D6E63"
      />
      <Path
        d="M261.477 142.495C270.086 140.8 274.851 126.123 272.12 109.713C269.389 93.303 260.196 81.3738 251.588 83.0686C242.979 84.7633 238.214 99.4401 240.945 115.85C243.676 132.26 252.869 144.189 261.477 142.495Z"
        fill="#8D6E63"
      />
      <Path
        d="M325.006 191.416H320.335C319.045 191.416 318 192.498 318 193.833V210.746C318 212.081 319.045 213.163 320.335 213.163H325.006C326.297 213.163 327.342 212.081 327.342 210.746V193.833C327.342 192.498 326.297 191.416 325.006 191.416Z"
        fill="white"
      />
      <Path
        d="M336.683 189H334.348C333.058 189 332.012 190.082 332.012 191.416V203.498C332.012 204.832 333.058 205.914 334.348 205.914H336.683C337.974 205.914 339.019 204.832 339.019 203.498V191.416C339.019 190.082 337.974 189 336.683 189Z"
        fill="white"
      />
      {/* 꼬리 - 기준점(185, 290) 중심으로 좌우 흔들림 */}
      <AnimatedG rotation={tailAngle} originX={185} originY={290}>
        <Path
          d="M185.437 290.317C121.213 332.602 97.8583 248.033 144.567 229.911C191.276 211.789 167.921 151.382 121.213 157.423"
          stroke="#8D6E63"
          strokeWidth={40}
          strokeLinecap="round"
        />
      </AnimatedG>
      {/* 왼팔 - 위아래 타자 애니메이션 */}
      <AnimatedG translateY={leftArmY}>
        <Path
          d="M230 217.8C263.333 233.933 293.333 252.083 320 272.25"
          stroke="#9C847B"
          strokeWidth={18}
          strokeLinecap="round"
        />
      </AnimatedG>
      <Path
        d="M225 338.8C255.376 338.8 280 298.17 280 248.05C280 197.93 255.376 157.3 225 157.3C194.624 157.3 170 197.93 170 248.05C170 298.17 194.624 338.8 225 338.8Z"
        fill="#9C847B"
      />
      <Path
        d="M240 326.7C259.33 326.7 275 294.196 275 254.1C275 214.004 259.33 181.5 240 181.5C220.67 181.5 205 214.004 205 254.1C205 294.196 220.67 326.7 240 326.7Z"
        fill="#D0C4C1"
      />
      <Path
        d="M190 177.753C206.667 133.386 235 121.286 275 141.453L345 171.703C358.333 178.156 358.333 184.206 345 189.853H258.5C231.833 193.886 206.667 197.92 190 177.753Z"
        fill="#9C847B"
      />
      <Path
        d="M230 163.35C249.33 163.35 265 147.098 265 127.05C265 107.002 249.33 90.75 230 90.75C210.67 90.75 195 107.002 195 127.05C195 147.098 210.67 163.35 230 163.35Z"
        fill="white"
      />
      <Path
        d="M278 147.08C293.464 147.08 306 134.078 306 118.04C306 102.002 293.464 89 278 89C262.536 89 250 102.002 250 118.04C250 134.078 262.536 147.08 278 147.08Z"
        fill="white"
      />
      {/* 오른쪽 눈 - 대각선 아래로 이동 */}
      <AnimatedG translateX={eyeX} translateY={eyeY}>
        <Path
          d="M245 154.84C252.732 154.84 259 147.256 259 137.9C259 128.544 252.732 120.96 245 120.96C237.268 120.96 231 128.544 231 137.9C231 147.256 237.268 154.84 245 154.84Z"
          fill="#4F5151"
        />
        <Path
          d="M240 136.69C242.209 136.69 244 134.523 244 131.85C244 129.177 242.209 127.01 240 127.01C237.791 127.01 236 129.177 236 131.85C236 134.523 237.791 136.69 240 136.69Z"
          fill="white"
        />
      </AnimatedG>
      {/* 왼쪽 눈 - 대각선 아래로 이동 */}
      <AnimatedG translateX={eyeX} translateY={eyeY}>
        <Path
          d="M288 144.62C294.075 144.62 299 138.661 299 131.31C299 123.959 294.075 118 288 118C281.925 118 277 123.959 277 131.31C277 138.661 281.925 144.62 288 144.62Z"
          fill="#4F5151"
        />
        <Path
          d="M284 130.1C285.657 130.1 287 128.475 287 126.47C287 124.465 285.657 122.84 284 122.84C282.343 122.84 281 124.465 281 126.47C281 128.475 282.343 130.1 284 130.1Z"
          fill="white"
        />
      </AnimatedG>
      <Path
        d="M355 192.62C363.837 192.62 371 186.661 371 179.31C371 171.959 363.837 166 355 166C346.163 166 339 171.959 339 179.31C339 186.661 346.163 192.62 355 192.62Z"
        fill="#4F5151"
      />
      <Path
        d="M398.609 169.978L394.746 168.726C392.612 168.034 390.418 169.566 389.847 172.148L366.035 279.675C365.463 282.257 366.73 284.911 368.864 285.603L372.727 286.855C374.861 287.547 377.055 286.015 377.626 283.433L401.438 175.906C402.009 173.324 400.743 170.67 398.609 169.978Z"
        fill="#B0BEC5"
      />
      <Path
        d="M391 278.3H304C301.791 278.3 300 280.467 300 283.14V290.4C300 293.073 301.791 295.24 304 295.24H391C393.209 295.24 395 293.073 395 290.4V283.14C395 280.467 393.209 278.3 391 278.3Z"
        fill="#90A4AE"
      />
      {/* 오른팔 - 위아래 타자 애니메이션 (왼팔과 교대) */}
      <AnimatedG translateY={rightArmY}>
        <Path
          d="M215 235.95C245 252.083 275 265.393 305 275.88"
          stroke="#9C847B"
          strokeWidth={20}
          strokeLinecap="round"
        />
      </AnimatedG>
      <Path
        d="M277.5 177C288.822 177 298 165.136 298 150.5C298 135.864 288.822 124 277.5 124C266.178 124 257 135.864 257 150.5C257 165.136 266.178 177 277.5 177Z"
        stroke="#1A1A1A"
        strokeWidth={5}
      />
      <Path
        d="M328 165C337.941 165 346 155.374 346 143.5C346 131.626 337.941 122 328 122C318.059 122 310 131.626 310 143.5C310 155.374 318.059 165 328 165Z"
        stroke="#1A1A1A"
        strokeWidth={5}
      />
      <Path
        d="M318 145.556C310.667 140.222 303.667 140.889 297 147.556"
        stroke="#1A1A1A"
        strokeWidth={5}
        strokeLinecap="round"
      />
      <Path
        d="M257 161C243.667 157.667 196.167 146 189.5 136"
        stroke="#1A1A1A"
        strokeWidth={5}
        strokeLinecap="round"
      />
      <Rect x={90} y={294} width={314} height={120} rx={5} fill="#71584F" />
    </Svg>
  );
}
